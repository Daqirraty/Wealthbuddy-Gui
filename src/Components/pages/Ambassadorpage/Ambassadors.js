...
import AmbassadorPops from '../../../ModalpopUP/AmbassadorModal'
import ContextModal from '../../../context/ContextModals'

const Ambassadors = () => {

    const { showContextAmbassadorModal, setshowContextAmbassadorModal } = useContext(ContextModal)
    ...
    return (
        <Container>
            ...
            {showContextAmbassadorModal ? <AmbassadorPops /> : null}
            ...
            <div className='container mx-auto w-[1000px] m-[5rem]'>
                ...
                <div className='border rounded-lg flex flex-rows w-[1000px] h-[58px] mb-5'>
                    ...
                    <button onClick={() => setshowContextAmbassadorModal(true)} className="bg-[#99BF18] w-[150px] h-[56px] text-white font-bold py-2 mb-2 px-4 border-2 rounded-lg hover:border-blue-400 inline-flex items-center justify-end">
                    ...
                    AmbassadorCards.map((AmbassadorCard) =>
                        ...
                        <div key={AmbassadorCard.id} className='flex justify-between w-[420.6px] h-[110.6px] m-5 ml-[5rem] border-2 rounded-[1.5rem] shadow-xl hover:border-blue-300'>
                            ...
                            <div className="flex ml-[3rem]">
                                <button onClick={() => setshowContextAmbassadorModal(true)} className="bg-[#99BF18] w-[90px] h-[46px] text-white font-bold py-2 m-[10px] px-6 mt-7 border-2 rounded-lg hover:border-blue-400 inline-flex items-center justify-end ">View</button>
                            </div>
                        </div>
                    )}
            </div>
